<?php
	...
	while ($users = mysql_fetch_assoc($result_users)) {
		...
		$indications = $obj = json_decode(file_get_contents('https://lk.waviot.ru/api/report/?template=a2f5261236bf3e2aede89cae168d2c2d&period=P1D&from='.$curdate.'&raw=1&modem='.$users['modem_num'].'&to='.$curdate));
		...
		for ($i=1; $i<=$count_tarifs; $i++) {
			...
			if (mysql_num_rows($check_tarif_result) != 0) {
				...
				//добавляем показания
				$insert_q = "INSERT INTO Indications SET date='".$curdate."', user=".$users['id'].", tarif=(SELECT id FROM tarifs WHERE id_waviot = '".$tarif."'), Indications=".$value.", prev_indications = '".$prev_ind."', additional=(SELECT price FROM tarifs WHERE id_waviot = '".$tarif."'), additional_sum= $ind_diff*(SELECT price FROM tarifs WHERE id_waviot = '".$tarif."'), auto = 1";
				//echo $insert_q;
