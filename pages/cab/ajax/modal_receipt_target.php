...
if (isset($_SESSION['id'])) {
  $amount = $db->getOne("SELECT balance FROM purses WHERE type = 3 AND user_id = ?i", $_SESSION['id']);
  //var_dump($amount);
  if ($amount >= 0) {
    $amount = '';
  } else {
  ...
  $json['header'] = 'Квитанция для оплаты целевых взносов';
  $json['html'] .= '<form>';
  ...
  $json['html'] .= '<button class="btn btn-success" onClick="window.open(\'/pages/cab/forms/?pay_target=\'+document.getElementById(\'electricAmount\').value, \'_blank\');" >Распечатать</button>';
